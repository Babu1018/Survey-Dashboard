...
class AuthentikAuth:
    ...
    def _register_routes(self) -> None:
        ...
        def me(payload: dict = Depends(self.current_user)) -> UserInfo:
            # The access token itself carries no role claim unless the
            # provider's scope mappings add one, so re-fetch the Authentik
            # user record here too (same as login) rather than returning
            # roles=[] on every session refresh.
            user = None
            email = payload.get("email")
            if email:
                user = self.client.find_user_by_email(str(email))
            return self._user_info(payload, user)
