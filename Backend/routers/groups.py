...
from schemas.survey import UserGroupSchema, SurveySchema
...
def create_group(group_data: UserGroupSchema, db: Session = Depends(get_db)):
...
def update_group(group_id: int, group_data: UserGroupSchema, db: Session = Depends(get_db)):
    group = db.query(UserGroup).filter(UserGroup.id == group_id).first()
    if not group:
        raise HTTPException(status_code=404, detail="Group not found")
...
def assign_survey(group_id: int, survey_id: int, db: Session = Depends(get_db)):
    group = db.query(UserGroup).filter(UserGroup.id == group_id).first()
...
def unassign_survey(group_id: int, survey_id: int, db: Session = Depends(get_db)):
    group = db.query(UserGroup).filter(UserGroup.id == group_id).first()
    survey = db.query(Survey).filter(Survey.id == survey_id).first()
...
def get_group_surveys(group_id: int, db: Session = Depends(get_db)):
    group = db.query(UserGroup).filter(UserGroup.id == group_id).first()
    if not group:
        raise HTTPException(status_code=404, detail="Group not found")
